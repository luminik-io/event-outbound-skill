 ...
 *
 * Reference: data/josh-braun-frameworks.md "Encoding plan §3" lists the 7 evals.
 */
...
const examples: CanonicalExamples = JSON.parse(
  readFileSync(
    join(process.cwd(), 'data', 'josh-braun-canonical-examples.json'),
    'utf-8',
  ),
);
    ...
    async () => {
      ...
      for (const ex of coldEmailPasses) {
        for (const T of ['Trigger', 'Think', 'Third-party', 'Talk?']) {
          const v = await judge({
            touch: ex,
            criterion: `Does this email contain a clear "${T}" component per the Josh Braun 4T framework?`,
            rubric:
              T === 'Trigger'
...
describe('Eval 4: Voice authenticity vs canonical', () => {
  it.skipIf(!judgeAvailable)(
    'each PASS cold-email scores >=3 for JB voice authenticity',
    async () => {
      const judge = makeGeminiJudge();
      const coldEmailPasses = examples.pass_examples.filter(
        (e) => e.touch_type === 'email_cold',
      ...
      for (const ex of coldEmailPasses) {
        const v = await judge({
          touch: ex,
          criterion: 'Does this email read in Josh Braun\'s voice?',
          rubric: `Reference: TitanX example by JB himself: "${titanx.body}". Score 0-5: short sentences, contrast pivots, lean-back CTA, no buzzwords, no self-praise.`,
        });
  ...
  it.skipIf(!judgeAvailable)(
    ...
    async () => {
      ...
      const vMulti = await judge({
        ...
        rubric: 'JB rule: isolate one problem per email. Mashing 2+ problems = fail.',
      });
      const vGood = await judge({
        touch: captivate,
        criterion: 'Does this email focus on exactly ONE problem?',
        rubric: 'JB rule: isolate one problem per email.',
      });
...
describe('Bonus: canonical PASS examples shouldn\'t trip critical validators', () => {
  ...
  it('every cold-email PASS (with one labeling-pattern exception) contains an illumination question', () => {
    ...
    // Note: solopreneur_tax uses JB's "presupposition + label" variant, not a how/what/why
    // question (synthesis doc Pass #5). It's intentionally a different pattern in JB's
