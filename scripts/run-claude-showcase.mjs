#!/usr/bin/env node
// Optional live Claude showcase runner.
//
...
// the checked-in examples/claude-showcase artifacts from real Claude runs.
...
const TMP = join(ROOT, '.tmp', 'claude-showcase-live');
...
for (let i = 0; i < args.length; i += 1) {
  ...
  if (arg === '--command') opts.command = args[++i];
    ...
    process.stdout.write(`Usage: node scripts/run-claude-showcase.mjs [options]

Options:
  --case <name>          Run one case from examples/claude-showcase
  ...
  --update-fixtures      Copy generated outputs back into examples/claude-showcase

Examples:
  npm run e2e:claude -- --case rich-positive-availability-unknown
  npm run e2e:claude -- --update-fixtures
`);
    process.exit(0);
  } else {
...
for (const caseName of cases) {
  ...
  writeFileSync(join(runDir, 'claude-output.txt'), result.stdout || '');
  if (result.stderr) writeFileSync(join(runDir, 'claude-stderr.txt'), result.stderr);
  if (result.status !== 0) {
  ...
  if (opts.updateFixtures) {
    copyFileSync(join(runDir, 'claude-output.txt'), join(caseDir, 'claude-output.txt'));
...
process.stdout.write(`Live Claude showcase runs written to ${TMP}\n`);
if (opts.updateFixtures) {
