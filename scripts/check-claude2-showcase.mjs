#!/usr/bin/env node
// Deterministic checks for checked-in Claude2 showcase outputs.
//
...
function checkGuardCase(caseName, required, forbidden = []) {
  ...
  const output = readText(dir, 'claude2-output.txt');
...
if (failures.length > 0) {
  process.stderr.write(`claude2 showcase checks failed:\n${failures.map((f) => `- ${f}`).join('\n')}\n`);
  process.exit(1);
}

process.stdout.write('claude2 showcase checks passed\n');
