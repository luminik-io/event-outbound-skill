...
| `Three Series B fintechs we worked with` / `Three Series C cybersecurity` / `Two Series A SaaS we partnered with` | Templated funding-stage aggregation. The shape `[number] Series [A-D] [industry] we [verb-past] their [metric] [%]` reads as customer proof but is interchangeable filler — any vendor could swap in any funding stage and any industry. If you actually have permission to cite a customer, name the customer; if you don't, drop the third-party validation and lean on the illumination question. Hard-banned phrases include `two/three/four/five/several/a few series a/b/c/d`, `series a/b/c fintechs we`, `series a/b/c saas we`, `series a/b/c cybersecurity`, `we worked with three`, `we partnered with two`, `we helped three`. |

**The right shape**: open with a specific observation about *this* recipient or *this* event. Use the recipient's company name, a public detail about their team, a specific session at the event, or a recent product/funding/hiring signal. If you don't have a specific anchor, reach for a situation trigger ("end of the month and lots of commissions to pay" sized to the recipient's role) before reaching for a generalization. Never open with the population shape. For third-party validation, name the actual customer (with permission) instead of a templated funding-stage aggregate.
