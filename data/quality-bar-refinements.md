...
>
> This file complements `josh-braun-frameworks.md` (frameworks) and `cold-email-data-report-stats.md` (quantitative benchmarks) with **what the validator and eval still don't catch**. Every requirement here is a place where the current skill output reads as competent but not as *the kind of email a top rep would actually send*.
>
> Scope: the items here go in a follow-up PR after the in-flight Josh Braun encoding (`feat/josh-braun-encoding`) lands. This file is the spec for that follow-up.
...
- **Validator (heuristic):** median sentence length already capped at ≤12 words in `josh-braun-frameworks.md` H-rules; tighten the **average** to ≤14 to catch "two short fragments balancing one bloated middle sentence."
...
- Pass the "scratch-your-head" test (Josh Braun's phrase): the reader should think *"I'm not sure what they mean — better keep reading"*
...
**Compare to a JB-style cost-of-inaction question:**
...
- **Spec extension:** add `cta_strategy` per touch type to `josh-braun-rules.json`:
  ```jsonc
  {
...
- [ ] `data/josh-braun-rules.json` extended with `cta_strategy_per_touch_type` block
- [ ] `data/josh-braun-rules.json` extended with `surface_level_question_blocklist`, `vague_filler_nouns`, `formal_register_openers`
...
The LinkedIn post we draft after this PR lands should NOT claim "100% Josh Braun compliant from day one." The honest story is:
...
2. *"I encoded Josh Braun's frameworks plus the Gong/30MPC 85M-email data report, added LLM-graded evals, refactored the sequencer to track sequence state across touches so pains don't recycle and CTA placement is strategic."*
