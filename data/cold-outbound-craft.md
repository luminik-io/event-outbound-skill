...
the cold-outbound canon's outbound philosophy is **"shine a light on a problem the prospect doesn't know they have, and let them discover the value themselves."** Prospects are biased against salespeople (they have "commission breath"), so persuasion ("we're the best", "10x your revenue") triggers what he calls the **zone of resistance**. The way out is the **4T framework**: a personally-relevant Trigger, a neutral Think question (an "illumination question") that makes the reader scratch their head, Third-party validation that does the boasting for you, and a single bridging sentence on how it works — closed with a low-pressure interest-based CTA, not a meeting ask.
...
|---|---|---|---|---|
| H1 | Subject line length | ≤ 4 words | `How_to_write_subject_lines_cold-outbound-archive.txt:17-22` | In CEB ✅ |
| H2 | Subject line case | All lowercase | (Lavender data + permission-based cadence) | In CEB ✅ |
...
| H5 | Email body length | 50-100 words, 3-4 sentences | `cold-email-archive.txt:561-563` ("literally four to five sentences") | In CEB ✅ |
| H6 | No moon-and-stars | Ban "if I could 10x your revenue", "if I told you something interesting", "would you be interested" lead-questions | `4t-framework-archive.txt:47-62` | **NEW — add to CEB banned phrases** |
| H7 | No biased self-claims | Ban "we're the best", "we're the only", "industry-leading", "award-winning" | `4t-framework-archive.txt:14-30`; `How_to_construct_an_effective_cold_email_cold-outbound-archive.txt:64-74` | Partially in CEB; add "we're the best", "we're the only", "award-winning" |
| H8 | No leading questions | Ban "If I could…would you be interested?" pattern (verb-phrase + "would you be interested") | `4t-framework-archive.txt:55-62` | **NEW heuristic** |
| H9 | No premature meeting ask | First touch must NOT include "15 minutes", "30 minutes", "book a call", "schedule a meeting", "calendar link" | `cold-email-archive.txt:608-625`; CTA-ranking data | In CEB (CTA ranking); add explicit phrase ban |
| H10 | No "happy to" sales cliché | "happy to send", "happy to share", "happy to chat" | (CEB inherited) | In CEB ⚠️ — but note the canon's author uses bare "happy to" in CSV (3 hits). Keep banned for skill output only when followed by send/share/chat/connect. |
...
| H14 | LinkedIn connection request length | ≤ 200 chars (≈25-40 words). Premium can go to 300, but default to 200 for portability. | LinkedIn platform constraint; cross-checked against short-form posts (`LinkedIn-CSV` posts under 50 words) | **NEW — channel-specific length rule** |
| H15 | LinkedIn DM (post-connect) length | 50-120 words. reference DM example is 32 words (`Josh, looks like your podcast episodes are under five minutes…`) (source: `LinkedIn-CSV:Sheet1`) | LinkedIn 1300-char ceiling, permission-based cadence | **NEW — channel-specific length rule** |
| H16 | LinkedIn day-of nudge | 30-60 words; reference a concrete time + place; one CTA | Inferred from the lean-back CTA principle (`cold-email-archive.txt:614-672`) | **NEW** |
...
|---|---|---|---|
| S1 | Trigger relates to the problem | The Trigger sentence and the Think sentence must "elegantly tie together." bad example: "Hey Josh notice you went to Florida State University how are you ensuring cold emails don't land in spam." (`4t-framework-archive.txt:199-204`) | 4T transcript |
| S2 | Question is genuinely neutral | The Think question must not betray the seller's preferred answer. Test: would the prospect feel "led" or "asked"? | `4t-framework-archive.txt:64-83`; `How_to_poke_the_bear_cold-outbound-archive.txt:40-50` |
| S3 | Third-party validation uses contrast | "94% vs 12%" is sharper than "94%". Canon: "It's the contrast that sells." | `4t-framework-archive.txt:213-222` |
| S4 | Customer language, not marketing language | Pull "before/after" copy verbatim from case-study quotes; avoid "award-winning"-style filler | `How_to_construct_an_effective_cold_email_cold-outbound-archive.txt:39-92` |
| S5 | Specific & crispy beats vague | "in 5 minutes instead of 5 hours" beats "60x faster"; "$23/month forgotten subscriptions" beats "save money" | `cold-email-archive.txt:561-583` |
| S6 | Lean-back CTA energy ("Regina-ish") | The CTA should feel uninterested in the deal. "Worth a look?" beats "Want to grab 15 min?" | `cold-email-archive.txt:614-672` |
| S7 | Address the primary objection inline | "Even if you have an accountant" — pre-empt the most likely "I already have one" rejection with a fragment | `cold-email-archive.txt:351-366` |
...
| S9 | Personal beats personalized when no signal | "End of month and lots of commissions to pay" (situation-true, not profile-true) is acceptable when no LinkedIn hook exists | `How_to_construct_an_effective_cold_email_cold-outbound-archive.txt:107-165` |

---

## The 4T framework (canonical email structure)

Every cold email has four parts, each starting with a T. Source: `4t-framework-archive.txt:116-243`.

### 1. Trigger — *Why this person, why now?*
A specific observation about the prospect that ladders back to what you sell. the canon lists ~9 trigger types; the canonical examples are:

- **Technology trigger**: "Looks like you're using Salesloft." (source: `4t-framework-archive.txt:147-156`)
- **Open-position trigger**: "Seems like you're doubling your outbound team over the next 12 months which suggests you're sending lots of cold emails." (source: `4t-framework-archive.txt:163-178`)
- **Hiring trigger**: "Notice you're hiring SDRs which suggests you might be sending lots of cold emails." (source: `4t-framework-archive.txt:175-178`)
- **Situation trigger** (when no profile signal): "End of the month and lots of commissions to pay." (source: `How_to_construct_an_effective_cold_email_cold-outbound-archive.txt:155-165`)

The deduction pattern is **A → which suggests B**. The trigger A is observable; the deduction B is the bridge to the problem. Canon: *"I'm taking that trigger event and I'm making a deduction. If A then B is probably true."* (source: `4t-framework-archive.txt:168-178`)

### 2. Think — *the illumination question*
A neutral question that shines a light on a problem the prospect probably doesn't know they have, designed to make them scratch their head and think *"I'm not sure — what do you mean?"* (source: `4t-framework-archive.txt:67-95`)

On what makes it work:
- Starts with **how / what / why are you** (`4t-framework-archive.txt:84-89`)
...
One sentence naming similar companies + a contrast number. Canon: *"We cannot toot our own horns. What we can do is let other people toot them for us."* (source: `4t-framework-archive.txt:206-225`)

Canonical example: *"Google and Salesforce are using us to deliver 94% of cold emails to inboxes compared to 12% before."* The contrast (94% vs 12%) is what sells. (source: `4t-framework-archive.txt:213-225`)

Then a **single bridging sentence** explaining how it works: *"It involves a warm-up tool that raises your inbox reputation to boost deliverability."* One sentence. Crispy. Specific. (source: `4t-framework-archive.txt:226-240`)

### 4. Talk? — *interest-based CTA, with a question mark*
Don't assume they want to talk. Test interest first.

canon-approved closers:
- "Worth a conversation?" (source: `How_to_construct_an_effective_cold_email_cold-outbound-archive.txt:285-290`)
- "Worth a look?" / "Worth a peek sometime?" (source: `cold-email-archive.txt:666-672`)
- "Worth an exchange?" (source: `cold-email-archive.txt:362-366`)
- "Open to learning more?" (source: `LinkedIn-CSV` 4T post)
- "Want to try it?" / "Think this might help?" (source: `cold-email-archive.txt:560`)
...
**Example B — CaptivateIQ** (`How_to_construct_an_effective_cold_email_cold-outbound-archive.txt`):
...
**Example C — TitanX** (canonical LinkedIn 4T post, source: `LinkedIn-CSV:Sheet1`):
...
**Example D — Truebill** (`cold-email-archive.txt:539-577`):
...
Source: `How_to_write_subject_lines_cold-outbound-archive.txt:1-130` and `cold-email-archive.txt:155-204`.

**Validator-friendly rules:**
- ≤ 4 words (subject lines longer than 4 perform worse) (source: `How_to_write_subject_lines_cold-outbound-archive.txt:17-22`)
- All lowercase, casual register (source: implied throughout, the canon writes "via Amanda" not "Via Amanda")
- No digits ("save 30%")
...
The Think sentence is the hardest part of the framework. these are called **illumination questions** — they "shine a light on a problem so other people can come to their own conclusions" (source: `4t-framework-archive.txt:84-100`).
...
4. **"How are you [current activity] without [hidden problem]?"**
   - "How do you know you're not overpaying for subscriptions you forgot about?" (variant) (cold-email-archive:545)
...
Source: `How_to_poke_the_bear_cold-outbound-archive.txt` (full transcript) + reinforcement in `4t-framework-archive.txt:62-95`.

**Definition.** A poke-the-bear question is a question that's *deliberately a little hard to answer* because it surfaces a problem the prospect hasn't fully thought through. The aim: get them to "scratch their head and think 'I'm not sure — what do you mean?'" (source: `How_to_poke_the_bear_cold-outbound-archive.txt:9-13`).

**Mechanics:**
1. Start with the **trigger context** (a presupposition the prospect will recognize as true).
2. Ask **how / what** — not yes/no.
3. Optionally offer **multiple choice** (A, B, or C) — this both makes it easier to answer AND signals you understand how the work actually gets done. (source: `How_to_poke_the_bear_cold-outbound-archive.txt:40-55`)
4. Make the last option *something they probably haven't tried* — Chris Voss's "illusion of control." (source: `How_to_poke_the_bear_cold-outbound-archive.txt:55-62`)

**Examples — good pokes:**
- "I speak with course creators all the time and they tell me they're losing 6-8% of revenue month-over-month from failed credit-card payments. How are you recovering failed payments — automation, folks reaching out, or an outsourced recovery team?" (source: `How_to_poke_the_bear_cold-outbound-archive.txt:23-37`)
...
Compiled from `Get_replies_for_cold_emails_cold-outbound-archive.txt`, `How_to_ask_question_to_get_replies_to_cold_emails_cold-outbound-archive.txt`, and the Lavender Live transcript.
...
Recommended skill-level patterns (adapted from permission-based voice but not literally his):
...
### PS line
Source: `cold-email-archive.txt:716-731`. A PS line that contains personalization that didn't fit the body increases reply rates. Useful for: a relevant LinkedIn post you read, a hobby, a mutual connection. Keep ≤ 1 sentence.

### Length compression
Brain can't comprehend many ideas at once: **isolate one problem per email; chain problems across the sequence** (source: `cold-email-archive.txt:758-789`). Triathlon-coach example: don't pitch swim + bike + run + nutrition + mental in one email. Use one per touch, sequenced.

---

## JTBD interview framework (for ICP analysis)

Source: `cold-outbound-archive_JTBD_Framework_for_Interviews.pdf`, full document.
...
### Tips the canon stresses (JTBD-pdf:p5)
...
Reference table per touch type. All ranges derived from canon material + platform constraints.
...
### LinkedIn DM canonical example (canonical annotation)
...
>
> the canon's gloss: *"No filler. No agenda. A gentle knock. They noticed something. Asked a question with humble curiosity. And left space for reflection. That's the essence of mindful outreach. No convincing. No pushing. No begging. An open hand, not a clenched fist."* (source: LinkedIn-CSV)
...
### Sampling method
- **Files:** `the cold-outbound canon - LinkedIn Posts - Sheet1.csv` (1000 rows) + `Sheet2.csv` (1000 rows) = 1998 non-empty posts
...
**Implication for skill:** the canon writes in *short, punchy sentences*. Half his sentences are ≤5 words. Validator could enforce **median sentence length ≤ 12 words** for any canon-flavored output.

### Phrases the canon uses repeatedly (frequency in 1998-post corpus)
...
### Phrases the canon never (or almost never) uses
...
The current CEB bans these phrases — but the canon's author uses them, sometimes regularly. Recommendation: **keep banned in skill output** (the CEB list is additive on top of the natural register), but be aware:

| Phrase | CEB status | the corpus hits | Recommendation |
|---|---|---|---|
| "happy to" | Banned | 3 | Keep banned for "happy to send/share/chat"; a bare "happy to" is fine but rare. Validator pattern: ban only `happy to (send\|share\|chat\|connect\|jump\|hop)`. |
| "no pressure" | Banned | 2 | Keep banned in opener position. the canon uses "low-pressure" (7) as adjective — distinct. |
| "—" (em-dash) | Banned per workspace `CLAUDE.md` | 382 in the corpus | Keep banned in agent output (workspace policy wins). permission-based voice survives via comma/period substitution. |

### Stylistic markers to preserve
- **Sentence fragments are OK** — the canon ends posts with fragments constantly ("Less force, more flow.")
...
> Steve — end of the month and lots of commissions to pay. curious how are you dealing with the labor-intensive error-prone process of hard pasting Excel pages into Google Sheets running commissions? Gong is using CaptivateIQ to run monthly commissions in 5 minutes instead of 5 hours. no rebuilding individual Google Sheets, manual entry, or customizing reports. worth the conversation?
>
> *Source: How_to_construct_an_effective_cold_email_cold-outbound-archive.txt, full email reconstructed.* Word count: 64. Sentences: 4.

**Pass #3 — TitanX (canonical LinkedIn 4T post)**
> Pete — looks like you have 9 SDRs cold calling Directors of Benefits and CHROs. how are you giving your reps more at bats? SDRs using TitanX have 6-8 conversations every 50 dials, compared to 1-2 before. we identify the people most likely to answer. no long-term contracts or new tech to implement. open to learning more?
>
> *Source: LinkedIn-CSV. literal canonical example.* Word count: 70.
...
>
> *Source: cold-email-archive:539-577.* Word count: 67.
...
>
> *Source: cold-email-archive:339-366. Real cold email the canon received and praised.* Word count: 53.
...
>
> **Failures:** Trigger and Think have nothing to do with each other (S1). The 4T framework collapses if the Trigger doesn't ladder to the problem. the canon calls this out explicitly (4T:199-204).
...
>
> **Failures:** Five problems crammed in (H19). Brain can't compute. Pick one. (cold-email-archive:758-773)
...
>
> **Failures:** Gating language (H17). the doctrine: attach or link the asset, never gate. (CEB)
...
Add to `cold-email-benchmarks.json` or a new `cold-outbound-rules.json`:
...
Create `evals/cold-outbound-evals.json` with:
...
- **Eval 4: Voice authenticity.** Does it sound permission-based? Score against the 5 canonical pass examples + the LinkedIn corpus voice markers (short sentences, contrast pivots, no buzzwords). 0-5 scale.
...
| File | Contents |
|---|---|
| `cold-email-benchmarks.json` (existing) | Stays as the cross-source baseline (Gong + 30MPC + Outbound Squad). No canon-specific patterns. |
| `cold-outbound-craft.md` (this file) | Canonical reference doc. Human-readable. Source of truth for the skill prompt. |
| `cold-outbound-rules.json` (NEW) | Machine-readable. Channel lengths, canon-specific banned phrases, illumination-question heuristic, leading-question regex. The validator merges this with `cold-email-benchmarks.json`. |
| `cold-outbound-canonical-examples.json` (NEW) | The pass/fail eval ground-truth, structured for the eval harness. |
...
4. **Reconcile the em-dash conflict.** Workspace policy bans em-dashes; the canon uses 382 of them. The skill must convert em-dashes to commas/periods when emitting, but keep the canon's voice cadence (short sentences, contrast pivots, fragment endings) intact.
...
| File | Lines | Used for |
|---|---|---|
| `4t-framework-archive.txt` | 244 | Core 4T structure, illumination-question patterns, banned anti-patterns |
| `Get_replies_for_cold_emails_cold-outbound-archive.txt` | 60 | Three-secret-questions framework |
| `How_to_ask_question_to_get_replies_to_cold_emails_cold-outbound-archive.txt` | 58 | Congruence principle for follow-ups |
| `How_to_construct_an_effective_cold_email_cold-outbound-archive.txt` | 296 | CaptivateIQ worked example, before/after copy mining |
| `How_to_poke_the_bear_cold-outbound-archive.txt` | 90 | Poke-the-bear technique, multiple-choice pattern |
| `How_to_write_subject_lines_cold-outbound-archive.txt` | 131 | All 8 subject-line patterns, ≤4 word rule |
| `cold-email-archive.txt` (Lavender Live transcript) | 925 | Multiple email examples (Truebill, solopreneur tax, lean-back CTA, multi-problem anti-pattern) |
| `cold-outbound-archive_JTBD_Framework_for_Interviews.pdf` | 9 pages | JTBD timeline, four forces of progress, customer-language doctrine |
| `the cold-outbound canon - LinkedIn Posts - Sheet1.csv` + `Sheet2.csv` | 1998 non-empty posts | Voice cadence, phrase frequency, banned-word verification, short-form length samples, real DM examples |
